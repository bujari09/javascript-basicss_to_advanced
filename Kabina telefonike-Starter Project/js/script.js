...
function thirr() {
  ...
  if (
    (numri.startsWith("044") || numri.startsWith("049")) &&
    numri.length === 9
  ) {
    intervalId = setInterval(function () {
      if (vleraLlogaris > 0) {
        ...
        info.innerText =
          "Llogaria juaj eshte harxhuar ju lutem rimbushni llogarin tuaj";
        llogaria.innerText = "Llogaria: $" + vleraLlogaris;
      }
    }, 1000);
  } else if (numri.startsWith("+1") && numri.length === 12) {
    return alert("Numri qe po therrasim eshte shenuar mire");
  } else {
    alert("Shkruani nje numer valid");
  }
}
